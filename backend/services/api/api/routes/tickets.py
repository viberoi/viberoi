"""Ticket detail + per-ticket session list."""
...
from viberoi_shared.sessions import list_sessions_for_ticket
...
async def detail_route(
    ...
    return TicketDetail(
        ...
        # placeholder until the Worker computes the rollup.
        total_sessions=0,
        total_cost_usd=Decimal("0.00"),
    )


@router.get("/{ticket_uuid}/sessions", response_model=SessionListResponse)
async def sessions_for_ticket_route(
    ticket_uuid: UUID,
    ctx: Annotated[
        ApiAuthContext,
        Depends(require_role(Role.ORG_ADMIN, Role.TEAM_LEAD, Role.DEVELOPER)),
    ...
    """Sessions whose branch parse attributed them to this ticket."""
    async with org_scoped_session(ctx.org_id) as db:
        t = await get_ticket(db, ticket_uuid)
        if t.org_id != ctx.org_id:
            raise NotFound(f"Ticket {ticket_uuid} not found")
        rows = await list_sessions_for_ticket(
            db, org_uuid=ctx.org_id, ticket_external_id=t.external_id
        )
    return SessionListResponse(
        items=[to_summary(r) for r in rows], next_cursor=None
    )
